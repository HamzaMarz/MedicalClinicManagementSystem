...
<div class="invoice-card">
    ...
    <div class="invoice-meta">
        <div style="color: black;">Invoice ID : <strong>#INV-{{ $patient_invoice->id }}</strong></div>
        <div style="color: black;">Issue Date : <strong>{{ $patient_invoice->created_at?->format('Y-m-d') ?? '--' }}</strong></div>
    </div>
    ...
    <div class="invoice-grid">
        ...
        <div class="invoice-item">
            ...
            <div class="value" style="margin-left:30px;">{{ $patient_invoice->appointment_id ?? '---' }}</div>
        </div>

        <div class="invoice-item">
            <div class="label">Patient</div>
            <div class="value">{{ $patient_invoice->appointment->patient->user->name ?? 'N/A' }}</div>
        </div>

        <div class="invoice-item">
            <div class="label">Total Amount</div>
            <div class="value">$ {{ number_format($patient_invoice->total_amount, 2) }}</div>
        </div>

        <div class="invoice-item">
            <div class="label">Discount</div>
            <div class="value">$ {{ number_format($patient_invoice->discount, 2) }}</div>
        </div>

        <div class="invoice-item">
            <div class="label">Final Amount</div>
            <div class="value">$ {{ number_format($patient_invoice->final_amount, 2) }}</div>
        </div>
    </div>

    @if($patient_invoice->items && $patient_invoice->items->count())
        ...
        <table class="invoice-table">
            ...
            <tbody>
                @php $total = 0; @endphp
                @foreach($patient_invoice->items as $index => $item)
    ...
    <div class="actions">
        <a href="{{ route('view_patients_invoices') }}" class="btn btn-back">Back</a>
